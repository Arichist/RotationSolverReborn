...
internal abstract class ActionGetterBase : ExcelRowGetter<Action>
{
    ...
    {
        var name = item.Name.ExtractText().ToPascalCase() + (item.IsPvP ? "PvP" : "PvE");
    ...
    {
        var desc = _gameData.GetExcelSheet<ActionTransient>()?.GetRow(item.RowId).Description.ExtractText() ?? string.Empty;
